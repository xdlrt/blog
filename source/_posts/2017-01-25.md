...
![A screenshot of tylergaw.com missing all styles](http://p0.qhimg.com/t015dc5a7c9cff0afee.png)
...
![A screenshot of tylergaw.com with partial styles](http://p0.qhimg.com/t0139ea99e96333843d.png)
...
[cssnext](http://cssnext.io/)是PostCSS中一个插件,用于将未来CSS特性编译为现今支持的特性.特别需要指出,它和Sass或Less并非不同的语言.它提供正在进行中的CSS规范的特性.一些特性已经得到浏览器支持.另外一些还处于规范的初始阶段.
...
嵌套是CSS预处理器不可或缺的特性.任何让人舒服的样式工具的必需品.Tab Atkins对CSS嵌套有一个正在进行中的[规范](http://tabatkins.github.io/specs/css-nesting/),并且cssnext让它成为现实.
...
另外值得注意的是`@nest`.正如[文档](http://cssnext.io/features/#nesting)中提到的,复杂的嵌套可能需要引入`@nest`来代替&.这个项目我还没有用到,或许将来用得到.
...
我在之前了解过CSS自定义属性的用法。这里有一个正在进行中的`@apply`规则的[规范](http://tabatkins.github.io/specs/css-apply-rule/)与之相关。`@apply`允许储存一系列的属性并且在选择器引用。我用`@apply`来代替Sass的`extend`.
...
![A screenshot of tylergaw.com/projects](http://p0.qhimg.com/t0102317bf30e69fdef.png)
...
为了样式按顺序回归,我决定利用更多的PostCSS插件.我用[css mqpacker](https://github.com/hail2u/node-css-mqpacker)来合并使用相同查询条件的媒体查询.我也用[cssnano](http://cssnano.co/)来优化代码.
